    ...
    ui(new Ui::MainWindow)
{
    ...
    this->setWindowTitle ("WorkObj Calibration");
     ...
     viewer->registerAreaPickingCallback(&MainWindow::AreaPickingEventOccurred, *this);
     ...
     high_curvature_edges.reset (new pcl::PointCloud<pcl::PointXYZRGBA>);


     ui->Filter_x_pos->setSingleStep(0.05);
     ui->Filter_x_neg->setSingleStep(0.05);
     ui->Filter_y_pos->setSingleStep(0.05);
     ui->Filter_y_neg->setSingleStep(0.05);
     ui->Filter_z_pos->setSingleStep(0.05);
     ui->Filter_z_neg->setSingleStep(0.05);

     Edge_detec_param = true;
...
void MainWindow::on_Edge_Detect_clicked()
{
    ...
    viewer->removePointCloud("high curvature edges");
    ...
    viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 4, "high curvature edges");
    viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_COLOR, 0.0f, 0.0f, 1.0f, "high curvature edges");


}


void MainWindow::on_Filter_Edge_clicked()
{

        QKeyEvent * eve1 = new QKeyEvent (QEvent::KeyPress,Qt::Key_X,Qt::NoModifier,"x");
        QKeyEvent * eve2 = new QKeyEvent (QEvent::KeyRelease,Qt::Key_X,Qt::NoModifier,"x");

        QCoreApplication::postEvent((QObject*)ui->qvtkWidget,(QEvent *)eve1);
        QCoreApplication::postEvent((QObject*)ui->qvtkWidget,(QEvent *)eve2);
...
/*
        ...
        high_curvature_edges_filtered_PT.reset (new pcl::PointCloud<pcl::PointXYZRGBA>);

        pass.setInputCloud (high_curvature_edges);
        pass.setFilterFieldName ("x");
        pass.setFilterLimits (-ui->Filter_x_neg->value(), ui->Filter_x_pos->value());
        //pass.setFilterLimits (-0.05, 0.25);
        pass.filter (*high_curvature_edges_filtered_PT);
        pass.setInputCloud (high_curvature_edges_filtered_PT);
        pass.setFilterFieldName ("y");
        pass.setFilterLimits (-ui->Filter_y_neg->value(), ui->Filter_y_pos->value());
        //pass.setFilterLimits (-0.035, 0.35);
        pass.filter (*high_curvature_edges_filtered_PT);


       // double min_dist=4;
        //    for (int i=0; i<high_curvature_edges_filtered_PT->size();++i)
        //    {  if (high_curvature_edges_filtered_PT->points[i].z<min_dist)
                   min_dist=high_curvature_edges_filtered_PT->points[i].z;
       //      }

        pass.setInputCloud (high_curvature_edges_filtered_PT);
        pass.setFilterFieldName ("z");
        pass.setFilterLimits (ui->Filter_z_neg->value(), ui->Filter_z_pos->value());
        //pass.setFilterLimits (min_dist-0.05, min_dist+0.015);
        pass.filter (*high_curvature_edges_filtered_PT);

        viewer->addPointCloud<pcl::PointXYZRGBA> (high_curvature_edges_filtered_PT, "high curvature edges");
        viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 4, "high curvature edges");
        viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_COLOR, 1.0f, 0.0f, 0.0f, "high curvature edges");
*/
...
void MainWindow::on_Compute_clicked()
{
    ...
    ui->WorkObj_Frame->clear();
     ...
     //high_curvature_edges.swap (high_curvature_edges_filtered_PT);
...
int MainWindow::on_Stop_Live_Stream_clicked()
{
    if (driver.getNumberDevices() == 0)
        QMessageBox::warning(this,tr("warning"),tr("No devices connected."));
    else{
...
/*
void MainWindow::AreaPickingEventOccurred(const pcl::visualization::AreaPickingEvent &event, void* viewer_void) {
        std::vector<int> indices;

        if (event.getPointsIndices(indices)) {
                if (indices.size() == 1)
                        std::cout << "Area selecionada: " << indices.size() << " ponto" << std::endl;
                else
                        std::cout << "Area selecionada: " << indices.size() << " pontos" << std::endl;
        } else
                std::cout << "Nenhum ponto selecionado" << std::endl;
}
*/
void MainWindow::AreaPickingEventOccurred(const pcl::visualization::AreaPickingEvent &event, void* viewer_void)
{
  ...
  cloud_filtered.reset(new pcl::PointCloud<pcl::PointXYZRGBA>);

  for(int i=0; i<indices.size(); i++){
      std::cout << cloud->points.at(indices.at(i)).x << "  " << cloud->points.at(indices.at(i)).y << "  "  << cloud->points.at(indices.at(i)).z << std::endl;
          cloud_filtered->points.push_back(cloud->points.at(indices.at(i)));
  }
  /*
  for (std::vector<int>::const_iterator it = indices.begin (); it != indices.end (); ++it)
  {
    std::cout << cloud->points[*it].x << "  " << cloud->points[*it].y << "  "  << cloud->points[*it].z << std::endl;
...
void MainWindow::on_Filter_x_pos_valueChanged(double arg1)
{
    viewer->removeShape ("cube", 0);
    viewer->addCube (-ui->Filter_x_neg->value(), ui->Filter_x_pos->value(), -ui->Filter_y_neg->value(), ui->Filter_y_pos->value(), ui->Filter_z_neg->value(), ui->Filter_z_pos->value(), 1.0, 0, 0, "cube", 0);

}

void MainWindow::on_Filter_x_neg_valueChanged(double arg1)
{
    viewer->removeShape ("cube", 0);
    viewer->addCube (-ui->Filter_x_neg->value(), ui->Filter_x_pos->value(), -ui->Filter_y_neg->value(), ui->Filter_y_pos->value(), ui->Filter_z_neg->value(), ui->Filter_z_pos->value(), 1.0, 0, 0, "cube", 0);
...
void MainWindow::on_Filter_y_pos_valueChanged(double arg1)
{
    viewer->removeShape ("cube", 0);
    viewer->addCube (-ui->Filter_x_neg->value(), ui->Filter_x_pos->value(), -ui->Filter_y_neg->value(), ui->Filter_y_pos->value(), ui->Filter_z_neg->value(), ui->Filter_z_pos->value(), 1.0, 0, 0, "cube", 0);
...
void MainWindow::on_Filter_y_neg_valueChanged(double arg1)
{
    viewer->removeShape ("cube", 0);
    viewer->addCube (-ui->Filter_x_neg->value(), ui->Filter_x_pos->value(), -ui->Filter_y_neg->value(), ui->Filter_y_pos->value(), ui->Filter_z_neg->value(), ui->Filter_z_pos->value(), 1.0, 0, 0, "cube", 0);
...
void MainWindow::on_Filter_z_pos_valueChanged(double arg1)
{
    viewer->removeShape ("cube", 0);
    viewer->addCube (-ui->Filter_x_neg->value(), ui->Filter_x_pos->value(), -ui->Filter_y_neg->value(), ui->Filter_y_pos->value(), ui->Filter_z_neg->value(), ui->Filter_z_pos->value(), 1.0, 0, 0, "cube", 0);
...
void MainWindow::on_Filter_z_neg_valueChanged(double arg1)
{
    viewer->removeShape ("cube", 0);
    viewer->addCube (-ui->Filter_x_neg->value(), ui->Filter_x_pos->value(), -ui->Filter_y_neg->value(), ui->Filter_y_pos->value(), ui->Filter_z_neg->value(), ui->Filter_z_pos->value(), 1.0, 0, 0, "cube", 0);
